...
addmargins(table(cond.supreg, cond.trend))
```
...
```{r}
set.seed(1)
out <- FloodnetPool(x = xd, target = mystation, period = 100, 
										distr = 'gev', out.model = TRUE, verbose = FALSE)
print(out$fit)
...
```{r}
set.seed(1)
out <- FloodnetPool(xd, target = mystation, period = 100, 
										verbose = TRUE, out.model = TRUE)
...
```{r}
xd <- with(descriptors, data.frame(
  	site = station,
    area = log(area),
    map  = log(map_ws),
    wb   = log(.01 + wb),
...
Please see the supplementary material `floodnetRfa_extra` that contain more code examples.
