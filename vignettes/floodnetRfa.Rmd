...
One objective of theme 1.6 from the project [FloodNet](http://www.nsercfloodnet.ca/) is to provide Canadian engineers and hydrologists with a set of tools that allows them to perform flood frequency analysis (FFA) easily and accurately.
...
In this document, we assume that the reader is familiar with the basic concepts of FFA  and will focus on the general capabilities of `floodnetRfa` within the terminal interface.
To illustrate common situations, the station 01AF009 located on the Iroquois River in New-Brunswick, serve as a case study.
...
Finally, the last type of function is interpreting functions that assess the component of the fitted models.
...
## Example of an unregular sequence.
...
When assuming that the exceeding probability is constant over time, the evaluation of a return period is equivalent to estimate the flood quantiles of probability $1-1/T$ for AMAX and $1-(\lambda T)^{-1}$ for POT.
...
Therefore GEV is prefered as it represent the asymptotic distribution of sample maxima.

The pipe operator `%>%` can be employed to obtain a syntax familiar to the [tidyverse](https://style.tidyverse.org/pipes.html).
The example below produce the same results as the earlier codes.
The rest of the document will adopt this syntax, because it is easier to read and avoid the creation of an intermediate variable.
...
The output of the modeling functions is a S3 object of the class `floodnetMdl` for which 
...
For for the 100-year flood quantile (Q100), the AMAX method estimates a streamflow of 98.1 $m^3/s$ with a standard deviation of 27.0. 
...
Therefore, it is important to verify that trends are not present in the initial data.
The function `floodnetAmax` automatically performs the Mann-Kendall's and Pettitt's test to examine the likelihood of trends or change points in the data (Helsel and Hirsch, 2002) and issues warnings it fails. 
...
By default, the return level plot is returned to compare the fitted flood quantiles (red line) with the ordered observations. 
...
Another plot for assessing the fitting of a distribution is the QQ-plot that displays the theoretical versus the sample quantiles of the distribution.
The QQ-plot differs from the return level plot by using a different x-axis, which should results in a linear relationship when a proper model is used.
...
Therefore, the same interpreting functions serve to extract information and create graphics.
...
More precisely, F-tests are applied to polynomials order 1 to 3 to detect significant trends from the logistic model and the lowest p-value is returned.
...
It is often interesting to try more than one model before making any final decision. In particular, by comparing the uncertainty of the estimated flood quantiles associated with each model.
...
Although it is common to consider administrative boundaries(_e.g._ provinces) in this situation, more hydrologically relevant super regions can be built by regrouping stations with similar characteristics. 
...
It includes the result of Mann-Kendall's (`trend_mk`) and Pettitt's (`trend_pt`) test that examines the annual maxima for potential trends and change points.
In the context of POT, the test of Mann-Kendall is applied to the exceedances (`trend_mx`) to verify potential changes in the mean excess. 
The presence of a trend in the exceedance rate is examined using logistic regression models on the daily data. F-tests are used to evaluate the significance of polynomial trends of order 1 to 3 versus a constant model. 
Finally, the minimal p-value is returned (`trend_lg`).
...
One consequence of this assumption is that the coefficient of variation all sites must be the same.
...
However, if the hydrometric data are coming from HYDAT, it is easier to use the function `DailyPeaksData` that performs as a single instruction the task of importing the hydrometric (`DailyData`) and and extracting the independent peaks (`ExtractPeaksData`).
...
When there is no hydrometric data at the site of interest, frequency analysis cannot be done simply by fitting a distribution. 
Instead, the quantile regression techniques (QRT) is a method that predicts flood quantiles based on some catchment descriptors.
...
The prediction of flood quantiles for one or more ungauged sites is performed using the function `FloodnetRoi`, which is built upon the function `FitRoi` in `CSHShydRology`.
...
Remember that we estimated Q100 equal to 98.1 $m^3/s$ using the AMAX method and we predict here 92.7 $m^3/s$ based on its catchment descriptors.
...
The regression model requires the calibration of a bandwidth parameter that controls the weight decay and the distance outside which the weights become null.
This parameter is expressed in `FloodnetRoi` as the rank of the nearest gauged sites to the target, which can be interpreted as the size of a region of influence (ROI) that contributes to the prediction of the target site.
...
After fitting the model, a 10-fold cross-validation resampling strategy is used to assess the quality of the model.
The strategy consists to divide the set of gauged sites into 10 groups of approximately equal sizes. 
In turn, each group is treated as ungauged sites and flood quantiles are estimated for each site without using the target information.
Note that the validation groups are divided at random and thus, the outcome of the cross-validation procedure will variate at each function call.

If more than one pooling group sizes are provided, the function determines the selected size by cross-validation.
The Nash-Sutcliffe criterion or NASH is a prediction skill score that rescales the mean square error into a unitless measure.
The latter is interpreted as the relative performance of model with respect to the sample average.
A NASH of 1 indicates a perfect score, while a score close to zero indicates the presence of a model that does not perform better than the benchmark model.
In the present context, the NASH is applied to the logarithm values to account for the scale of the watersheds. 
...
It has the same forms as the NASH, except that the square differences are replaced by absolute deviations.
At the end of the cross-validation procedure, the selected pooling group size is the one that maximize the SKILL score among the list of candidates.

The results of the cross-validation procedure can be displayed using the function `summary`.
Please note in the output below the small discrepancies between the two `nash` criteria that result from the use of different cross-validation samples for the calibration step and the final evaluation of the model performance.  
...
The two following graphics can also be used to assess the estimated flood quantiles and the calibration of the pooling group size.
...
Deviation from the unitary line would suggest the presence of systematic errors in the QRT model.
The second plot shows the evolution of two cross-validation scores with respect to the pooling group size. 

To estimate the modeling uncertainty, the function `FloodnetRoi` uses a bootstrap resampling technique.
...
If coordinates are provided to the function `FloodnetRoi`, it will further perform a simple kriging technique on the residual of the QRT to improve the prediction of the flood quantiles. 
...
For more code and examples related to the R-package `floodnetRfa`, please see the supplementary material in `floodnetRfa_extra`.
