...
- Before starting to use the GUI, the user must make sure that he has collected the following information.
...
- Peaks are exceedances extracted from a declustering method based on a given threshold and the basin drainage area
...
- An homogenous region is a neighborhood (pooling group) formed around a target site.
...
- Heterogeneous sites are sequentially removed from an initial pooling group until homogeneity is reached.
	
- However this process will be stopped if the number of station-year fall below $5T$, where $T$ is the return period.

## Super regions

- A super region represents a classification of all stations found in station data file.

- It can be used to filter the stations that will be used to form the pooling group.

- A simple example of super region would be Canadian provinces.

- However, floodnet recommends to use a classification based on the drainage area and the mean annual precipitation.

## Super regions

- The GUI does not include a procedure to create the super regions, but the R terminal does.
- The example below creates `N` super regions using the Ward's method.

```{r, eval = FALSE}
csv <- read.csv(file = "station_data.csv")

# add a column containing the super regions
csv$supreg_new <- 
    dist(csv[,c('area','map')]) %>%
    hclust(method = 'ward') %>%
    cutree(N)

write.csv(csv, file = "station_data_modif.csv")
```
...
- This image presents an overlook of the model screen that is used for configuring and managing models.
...
- The application need two sources of data.
	- Hydrometric data.
	- Station data.
	
-  Hydrometric data
	- Either a CSV file or a local HYDAT database (sqlite3)
...
- Example of a station data file.

<img src="fig_gui/excel.PNG" style="height: 300px; width: auto;"/>
...
- The drainage area is read from the column `area` in the station data file .

<img src="fig_gui/model_configuration_pot_mod.PNG" style="height: 250px; width: auto;"/>
